 ...
 *  6. Client attaches the token as "Authorization: Bearer <token>" on each request
 *  7. JwtAuthFilter intercepts, validates, and sets the SecurityContext
 *  8. Protected endpoints are resolved by Spring Security using the SecurityContext
 * ================================================================
 */
public class AuthDTO {
    ...
    public static class RegisterRequest {
         ...
         */
        @Pattern(
            regexp = "^(USER|ADMIN|TECHNICIAN)$",
            message = "Role must be one of: USER, ADMIN, TECHNICIAN"
        )
     ...
     * 'expiresIn' indicates token validity in seconds (matches app.jwt.expiration-ms / 1000).
     */
    @Data
